import { useAppStore } from '@/stores/appStore';
import { LEFT_SIDEBAR_ITEMS } from '@/constants/apps';
import { X } from 'lucide-react';

const LeftSidebar: React.FC = () => {
  const { leftSidebarOpen, toggleLeftSidebar } = useAppStore();

  return (
    <div
      className={`fixed left-0 top-0 bottom-0 z-40 flex flex-col transition-transform duration-300 ease-out ${
        leftSidebarOpen ? 'translate-x-0 slide-in-left' : '-translate-x-full'
      }`}
      style={{
        width: 80,
        background: 'rgba(255,255,255,0.94)',
        backdropFilter: 'blur(24px)',
        WebkitBackdropFilter: 'blur(24px)',
        borderRight: '1px solid rgba(255,255,255,0.7)',
        boxShadow: '4px 0 24px rgba(0,0,0,0.1)',
        maxWidth: '100vw',
      }}
    >
      <div className="flex items-center justify-between px-2 py-3 border-b border-white/50">
        <span className="text-xs font-bold text-gray-600">Tools</span>
        <button onClick={toggleLeftSidebar} className="p-1 rounded-full hover:bg-gray-100">
          <X size={14} className="text-gray-500" />
        </button>
      </div>

      <div className="flex-1 overflow-y-auto scrollbar-hide py-2">
        {LEFT_SIDEBAR_ITEMS.map((item) => (
          <button
            key={item.id}
            className="w-full flex flex-col items-center gap-1 px-2 py-2.5 hover:bg-white/60 transition-colors rounded-lg mx-1"
            style={{ width: 'calc(100% - 8px)' }}
          >
            <span className="text-xl">{item.icon}</span>
            <span className="text-[9px] text-gray-500 font-medium text-center leading-tight">{item.label}</span>
          </button>
        ))}
      </div>
    </div>
  );
};

export default LeftSidebar;
